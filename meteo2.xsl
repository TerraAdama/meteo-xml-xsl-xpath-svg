...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%">
            ...
            <xsl:for-each select="meteo/mesure[@date='2024-11-11']/ville">
                <xsl:variable name="pos" select="position()*120"/>
                <xsl:variable name="temp" select="@temperature*10"/>
                <text stroke="blue" x="{$pos}" y="620">
                    <xsl:value-of select="@nom"/>
                </text>
                <rect width="20" height="{$temp}"  x="{$pos}" y="{600 - $temp}" stroke="blue" fill="orange"></rect>
                <text stroke="blue" x="{$pos}" y="{600 - $temp - 15}">
